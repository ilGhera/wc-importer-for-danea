...
class WCIFD_Import_Products {
	...
	private function css_handler() {
		...
		foreach ( $products as $product ) {
			...
			$this->csv_single_product( $product );
	...
	/**
	 * Prepare data of the single CSV product
	 *
	 * @param array $product the product data received by CSV.
	 *
	 * @return array
	 */
	private function prepare_product_data( $product ) {
		...
		$data                 = array();
		$data['sku']          = $product['Cod.'];
		$data['title']        = $product['Descrizione'];
		$data['description']  = isset( $product['Descriz. web (Sorgente HTML)'] ) ? $product['Descriz. web (Sorgente HTML)'] : '';
		$data['product_type'] = isset( $product['Tipologia'] ) ? $product['Tipologia'] : '';
		$data['category']     = isset( $product['Categoria'] ) ? $product['Categoria'] : '';
		$data['sub_category'] = isset( $product['Sottocategoria'] ) ? $product['Sottocategoria'] : '';
		$data['tax']          = isset( $product['Cod. Iva'] ) ? $product['Cod. Iva'] : '';

		if ( 0 === intval( $tax_included ) ) {
			$data['regular_price'] = str_replace( ',', '.', str_replace( array( ' ', '€' ), '', $product[ 'Listino ' . $get_regular_price_list ] ) );
			$data['sale_price']    = str_replace( ',', '.', str_replace( array( ' ', '€' ), '', $product[ 'Listino ' . $get_sale_price_list ] ) );

		} else {
			$data['regular_price'] = str_replace( ',', '.', str_replace( array( ' ', '€' ), '', $product[ 'Listino ' . $get_regular_price_list . ' (ivato)' ] ) );
			$data['sale_price']    = str_replace( ',', '.', str_replace( array( ' ', '€' ), '', $product[ 'Listino ' . $get_sale_price_list . ' (ivato)' ] ) );
		}

		$data['supplier_id']       = isset( $product['Cod. fornitore'] ) ? $product['Cod. fornitore'] : '';
		$data['supplier']          = isset( $product['Fornitore'] ) ? $product['Fornitore'] : '';
		$data['parent_sku']        = null;
		$data['variable_product']  = null;
		$data['parent_product_id'] = null;
		$data['var_attributes']    = null;
		...
		if ( $product['Note'] ) {
			...
			if ( isset( $notes['parent_sku'] ) && '' !== $notes['parent_sku'] ) {
				...
				$data['parent_sku'] = $notes['parent_sku'];

			} elseif ( isset( $notes['parent_id'] ) && '' !== $notes['parent_id'] ) {

				$data['parent_product_id'] = $notes['parent_id'];

			}

			if ( $data['parent_sku'] ) {
				$data['parent_product_id'] = WCIFD_Functions::search_product( $data['parent_sku'] );
			}

			$data['var_attributes'] = isset( $notes['var_attributes'] ) ? $notes['var_attributes'] : '';
			$data['var_type']       = isset( $notes['product_type'] ) ? $notes['product_type'] : '';

			/*Prodotto variabile*/
			if ( 'variable' === $data['var_type'] && $notes['attributes'] ) {
				$data['variable_product']    = true;
				$data['imported_attributes'] = $notes['attributes'];
			}
		}

		/*Post status - Le variazioni devono essere pubblicate*/
		$data['new_products_status'] = get_option( 'wcifd-publish-new-products' ) ? 'publish' : 'draft';
		$data['status']              = ( $data['var_attributes'] ) ? 'publish' : $data['new_products_status'];

		/*Verifico la presenza del prodotto*/
		$data['id']   = WCIFD_Functions::search_product( $data['sku'] );
		$data['type'] = ( wp_get_post_parent_id( $data['id'] ) || $data['parent_product_id'] ) ? 'product_variation' : 'product';

		/*Gestione magazzino*/
		$data['stock']        = $product['Q.tà giacenza'];
		$data['total_sales']  = $product['Tot. q.tà scaricata'];
		$data['manage_stock'] = ( 'Art. con magazzino' === $data['product_type'] || 'Art. con magazzino (taglie/colori)' === $data['product_type'] ) ? 'yes' : 'no';

		/*Dimensione prodotto*/
		$data['length'] = WCIFD_Functions::get_product_size( $product, $size_type, 'z', true );
		$data['width']  = WCIFD_Functions::get_product_size( $product, $size_type, 'x', true );
		$data['height'] = WCIFD_Functions::get_product_size( $product, $size_type, 'y', true );

		/*Peso del prodotto*/
		if ( 'gross-weight' === $weight_type ) {
			$data['weight'] = isset( $product['Peso lordo'] ) ? $product['Peso lordo'] : '';
		} else {
			$data['weight'] = isset( $product['Peso netto'] ) ? $product['Peso netto'] : '';
		}

		/*Autore del post come fornitore*/
		$data['author'] = ( 1 === intval( $use_suppliers ) && $data['supplier_id'] ) ? $data['supplier_id'] : get_option( 'wcifd-current-user' );
	...
	/**
	 * Handles the single CSV product
	 *
	 * @param array $product the product data received by CSV file.
	 *
	 * @return void
	 */
	private function csv_single_product( $product ) {
